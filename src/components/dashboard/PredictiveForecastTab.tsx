    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2.5 text-xs font-mono bg-white p-2.5 sm:p-3 rounded-xl border border-slate-200 shadow-2xs">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="font-bold text-slate-900 block text-xs sm:text-sm">AI/ML Subsidence Predictive Engine</span>
            <span className="text-slate-500 text-[10px] sm:text-[11px]">Polynomial Autoregressive Strata Forecasting (1h – 6h Horizon)</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 font-mono text-xs">
          ...
          <div className="rounded-lg bg-slate-50 border border-slate-200 px-2.5 py-1 flex items-center gap-1 text-[11px]">
            ...
            <span className="text-slate-900 font-bold">Polynomial Strata</span>
          </div>
        </div>
      </div>

      {/* AI Early Warning Advisory Banner */}
      <div className={`rounded-xl border p-3 sm:p-3.5 font-mono text-xs flex items-start gap-2.5 shadow-2xs ${
        isCritical 
        ...
        {isCritical ? (
          <AlertOctagon className="h-4 w-4 text-rose-600 shrink-0 mt-0.5 animate-pulse" />
        ) : isAccelerating ? (
        ...
        <div className="space-y-0.5">
          <div className="font-bold uppercase tracking-wider text-xs sm:text-sm flex items-center gap-2">
            <span>AI EARLY WARNING: {forecast.riskTrend.replace('_', ' ')}</span>
            <span className="text-[9px] px-1.5 py-0.2 rounded bg-white/80 border border-slate-300 shadow-2xs font-semibold">
              Confidence {forecast.confidenceScorePct}%
            </span>
          </div>
          <p className="leading-relaxed text-slate-700 font-medium text-[11px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 font-mono">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-2xs space-y-0.5">
          <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold block">Current Ground Sag</span>
          <div className="flex items-baseline justify-between">
            <span className="text-xl font-bold text-slate-900">{forecast.currentSagCm.toFixed(1)} cm</span>
            <span className="text-[11px] text-slate-500 font-medium">Base: 22.0cm</span>
          </div>
        ...
        {/* 2. Projected 3h Sag */}
        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-2xs space-y-0.5">
          <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold block">AI Projected (+3 Hours)</span>
          <div className="flex items-baseline justify-between">
            <span className={`text-xl font-bold ${forecast.projectedSag3hCm >= 4.0 ? 'text-rose-600' : 'text-amber-600'}`}>
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-2xs space-y-0.5">
          <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold block">Deformation Velocity</span>
          <div className="flex items-baseline justify-between">
            <span className="text-xl font-bold text-sky-600">
              {forecast.currentVelocityCmPerHour.toFixed(2)} cm/hr
            </span>
            <span className="text-[11px] text-slate-500 font-medium">Accel: {forecast.accelerationCmPerHour2.toFixed(3)}</span>
          </div>
          <div className="text-[9px] text-slate-400 pt-1 border-t border-slate-100 font-medium">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-2xs space-y-0.5">
          <span className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold block">Est. Time to Critical</span>
          <div className="flex items-baseline justify-between">
            <span className={`text-xl font-bold ${forecast.estimatedTimeToCriticalHours !== null && forecast.estimatedTimeToCriticalHours <= 3.0 ? 'text-rose-600' : 'text-emerald-700'}`}>
          ...
          <div className="flex items-center gap-2.5">
            <div className="flex items-center gap-3 text-[10px]">
              <span className="flex items-center gap-1 text-sky-700 font-bold">
                <span className="inline-block w-2.5 h-0.5 bg-sky-600" /> Observed
              </span>
              <span className="flex items-center gap-1 text-amber-700 font-bold">
                <span className="inline-block w-2.5 h-0.5 bg-amber-600 border-dashed" /> AI Projected
              </span>
              <span className="flex items-center gap-1 text-purple-700 font-semibold hidden md:inline-flex">
                <span className="inline-block w-2 h-1.5 bg-purple-100 border border-purple-300" /> 95% Band
              </span>
            </div>
            ...
            <div className="inline-flex rounded-md bg-slate-100 p-0.5 border border-slate-200">
              <button
                ...
                className={`px-2.5 py-0.5 rounded text-xs font-semibold transition-all ${forecastHorizon === '3h' ? 'bg-white text-slate-900 shadow-2xs' : 'text-slate-600 hover:text-slate-900'}`}
              >
                +3h View
              </button>
              <button
                onClick={() => setForecastHorizon('6h')}
                className={`px-2.5 py-0.5 rounded text-xs font-semibold transition-all ${forecastHorizon === '6h' ? 'bg-white text-slate-900 shadow-2xs' : 'text-slate-600 hover:text-slate-900'}`}
              >
                +6h View
              </button>
            </div>
